...
/*
  ...
  复制nmat的第四列和最后俩列,按住ctrl加选哈；记住有多少行,将来要写入C++第一个宏定义的位置
...
#define MAX 20           //单一时间区间最多可容纳的音符个数(和电机数量无关),一般不修改,如果遇到黑乐谱那就改大点
#define FREQUENCY 62200   
#define _CRT_SECURE_NO_WARNINGS
static int k;//区间个数
static int NOTE_NUMBER;//音符数量
...
void line_counter(char *midi_file/*统计行数，也就是音符数量*/){
...
void main(){
	int i=0,j=0;
	float total_time=0;
	line_counter(MIDI);//读取行数
	float *time=new float[NOTE_NUMBER*4];//工具数组,用一下就不用了
	note *note_INFO=new note[NOTE_NUMBER]; //记录每一个音符数据
